...
class Nav2D(QWidget):
    ...
    def update_style(self, palette=None):
        ...
        self.fg15 = QPen(color)
        self.fg15.setWidthF(1.5)
        color = palette.color(QPalette.WindowText)
        self.fg05 = QPen(color)
        self.fg05.setWidthF(0.5)
        color = palette.color(
            QPalette.Active, QPalette.Highlight)
        self.hg = QBrush(color)
        color = palette.color(QPalette.Base)
        self.bsbr = QBrush(color)
...
if __name__ == "__main__":
    ...
    class MainWindow(QMainWindow):
        def __init__(self, parent=None):
            ...
            size = self.size()
            self.nav2d.setGeometry(0, 0, size.width(), size.height())
